        ...
        >
          ...
          ${isExpanded ? 'bg-sienna translate-x-0' : 'bg-white'}`}
                >
                    ...
                    >
                        <Plus color={isExpanded ? 'white' : '#2D1810'} size={30} />
                    </MotiView>
                </TouchableOpacity>
                        ...
                        >
                            ...
                            <View className="flex-row bg-sienna/95 px-3 py-2 rounded-full border border-white/10 items-center justify-center gap-1.5 h-14">
                                <ToolIconButton
                                    ...
                                    icon={settings.torchOn ? <Zap color="#D4AF37" size={20} /> : <ZapOff color="white" size={20} />}
                                />
                                <ToolIconButton
                                    active={settings.showGrid}
                                    onPress={() => toggleTool('showGrid')}
                                    icon={<Grid3X3 color={settings.showGrid ? '#D4AF37' : 'white'} size={20} />}
                                />
                                <ToolIconButton
                                    active={settings.showReference}
                                    onPress={() => toggleTool('showReference')}
                                    icon={<ImageIcon color={settings.showReference ? '#D4AF37' : 'white'} size={20} />}
                                />
                            </View>
                            ...
                            <View className="bg-sienna/95 p-5 rounded-[2.5rem] border border-white/5 shadow-2xl space-y-5 w-44">
                                ...
                                <View className="flex-col items-center gap-1 opacity-80 pt-2">
                                    <NudgeBtn onPress={() => nudge(0, -1)} icon={<ChevronUp color="white" size={14} />} />
                                    <View className="flex-row gap-1">
                                        <NudgeBtn onPress={() => nudge(-1, 0)} icon={<ChevronLeft color="white" size={14} />} />
                                        <NudgeBtn onPress={resetTransform} icon={<RotateCcw color="#D4AF37" size={14} />} />
                                        <NudgeBtn onPress={() => nudge(1, 0)} icon={<ChevronRight color="white" size={14} />} />
                                    </View>
                                    <NudgeBtn onPress={() => nudge(0, 1)} icon={<ChevronDown color="white" size={14} />} />
                                </View>
                            </View>
                        </MotiView>
                    )}
